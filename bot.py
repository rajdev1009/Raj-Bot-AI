...
async def log_conversation(client, message, bot_reply):
    """
    Ye function chat ko 2 jagah save karta hai:
    1. Koyeb Dashboard (Console)
    2. Private Log Channel (Telegram)
    """
    user = message.from_user
    chat_text = message.text or "[Media/Sticker]"
    time_now = datetime.now().strftime("%d-%m-%Y %I:%M %p")
    
    # 1. Koyeb Dashboard Log
    logger.info(f"📩 User: {user.first_name} | Msg: {chat_text}")
    logger.info(f"📤 Bot: {bot_reply[:50]}...") # Dashboard me bas shuruwat dikhegi
...
async def image_gen_handler(client, message):
    ...
    if image_url:
        ...
        await log_conversation(client, message, f"Image Generated: {prompt}")
    else:
...
async def text_handler(client, message):
    ...
    # 1. SECURITY CHECK (Keyword: 'raj' - No slash)
    if text_lower == "raj":
    ...
    if cached_ans:
        final_ans = f"{cached_ans}\n\n(Saved Memory 🧠)"
        await message.reply_text(final_ans, reply_markup=speaker_btn)
    ...
    if "dev" in text_lower:
        ...
        if ai_response:
            ...
            await log_conversation(client, message, "AI Failed/Busy")
    
    # 5. IGNORE (Agar na Raj, na Hi, na DB, na Dev)
    else:
        pass
...
async def speak_callback_handler(client, callback_query: CallbackQuery):
    ...
    # Text nikalo (Message se ya Caption se)
    text_to_speak = callback_query.message.text or callback_query.message.caption
    
    # Clean text (Saved Memory tag hatao bolne ke liye)
    if text_to_speak:
        text_to_speak = text_to_speak.replace("(Saved Memory 🧠)", "").strip()
    ...
    try:
        ...
        await callback_query.answer("❌ Audio error aa gaya.", show_alert=True)
...
async def voice_handler(client, message):
    ...
    speaker_btn = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔊 Suno", callback_data="speak_msg")]
    ])
...
async def main():
    ...
    logger.info("🚀 Raj Bot (Logs + Speaker) Start Ho Raha Hai...")
    await app.start()
    await idle()
    await app.stop()
